classdef GCS_PSO_NBC< ALGORITHM
    ...
    methods
        function main(Algorithm,Problem)
            ...
            while Algorithm.NotTerminated(Population)
                ...
                Output = Population(front==1);
                %% database
                [~,index]  = unique(Population.decs,'rows');
                Population = Population(index);
                randIndex = randperm(length(Population));  % 打乱
                ...
                for i = 1 : Problem.M
                    Model{i} = fitcnb(PopDec,GLoc(:,i),'DistributionNames','kernel');  % 分布采用高斯核平滑密度估计Gaussian Kernel smoothing density estimate 
                ...
                while w < wmax
                    ...
                    for i = 1 :Problem.M
                        OffGLoc(:,i) = predict(Model{i},OffDec);
                    end
                ...
                if n <= 5
                    ...
                    choose = EnvironmentalSelectionNSGA2(Population,5); % 选最优的5个作为参考点
                    Ref = PopDec(choose,:);
                    Location = zeros(1,n);
                    for i = 1 : n
